...
// =====================================================
// ALERT API HELPERS (DATABASE BACKED)
// =====================================================
...
async function getAlerts() {
    ...
    try {
        ...
        // Backend returns: id, email, query, target_price, is_active
        // Map to frontend format if needed, or update frontend to use backend format
        // Frontend expects: id, product (query), targetPrice, active, createdAt (optional)
        return alerts.map(a => ({
            id: a.id,
            product: a.query,
            targetPrice: a.target_price,
            active: a.is_active,
            createdAt: new Date().toISOString() // Backend doesn't return created_at in list yet, using current time as fallback or ignore
        }));
    } catch (e) {
...
/**
 * Save alerts (Deprecated/No-op for backend)
 */
function saveAlerts(alerts) {
    // No-op since we rely on backend
}
